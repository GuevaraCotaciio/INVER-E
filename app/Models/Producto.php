...
class Producto extends Model
{
    ...
    function ID_Producto(){
        try {
            $IDProduc = DB::table('producto')->select('id')->max('id');
            return $IDProduc + 1;
        } catch (\Exception $e) {
            throw $e;
    ...
    public function Guardar_Producto($datos, $idP, $estado){  //guardo los datos
        try{
    ...
    public function Actualizar_Producto($datos, $idP, $estado){   // actualizo los datos
        try{
            $producto = new Producto();
            $producto->id                      = $idP;
            $producto->id_proveedor            = $datos->IDproveedor;
    ...
    public function Buscar_Producto_ID($IdProducto){
